...
class CausalConv1d(torch.nn.Conv1d):
    ...
    def forward_with_cache(
        ...
        if self.causal_type != 'left':
            return self.forward(x, cache), torch.zeros(0, 0, 0, dtype=x.dtype, device=x.device)
        input_timestep = x.shape[2]
        if commit_len is None:
            commit_len = input_timestep
        commit_len = max(0, min(int(commit_len), input_timestep))
        if cache.size(2) == 0:
            cache = torch.zeros(x.shape[0], x.shape[1], self.causal_padding).to(x)
        else:
            cache = cache.to(device=x.device, dtype=x.dtype)
        assert cache.size(2) == self.causal_padding
        x_in = torch.concat([cache, x], dim=2)
        y = super(CausalConv1d, self).forward(x_in)
        assert y.shape[2] == input_timestep
        committed = torch.concat([cache, x[:, :, :commit_len]], dim=2)
        new_cache = committed[:, :, -self.causal_padding:].detach() if self.causal_padding > 0 else cache[:, :, :0]
        return y, new_cache
...
class CausalConv1dDownSample(torch.nn.Conv1d):
    ...
    def forward_with_cache(
        ...
        assert cache.size(2) == self.causal_padding
        x_in = torch.concat([cache, x], dim=2)
        y = super(CausalConv1dDownSample, self).forward(x_in)
        committed = torch.concat([cache, x[:, :, :commit_len]], dim=2)
        new_cache = committed[:, :, -self.causal_padding:].detach() if self.causal_padding > 0 else cache[:, :, :0]
        return y, new_cache
...
class CausalConv1dUpsample(torch.nn.Conv1d):
    ...
    def forward_with_cache(
        ...
        commit_up = self.upsample(x[:, :, :commit_len])
        input_timestep = x_up.shape[2]
        if cache.size(2) == 0:
            cache = torch.zeros(x_up.shape[0], x_up.shape[1], self.causal_padding).to(x_up)
        else:
            cache = cache.to(device=x_up.device, dtype=x_up.dtype)
        assert cache.size(2) == self.causal_padding
        x_in = torch.concat([cache, x_up], dim=2)
        y = super(CausalConv1dUpsample, self).forward(x_in)
        assert input_timestep == y.shape[2]
        committed = torch.concat([cache, commit_up], dim=2)
        new_cache = committed[:, :, -self.causal_padding:].detach() if self.causal_padding > 0 else cache[:, :, :0]
        return y, new_cache
